...
The APIs support rate limiting with 50 RPM (requests per minute) per client across all the APIs. Learn more about [rate limiting](../../guides/concepts/rate-limits/index.md) guidelines. **Note:** If the rate limit is exceeded, any additional requests will receive a `429 Too Many Requests` response.
