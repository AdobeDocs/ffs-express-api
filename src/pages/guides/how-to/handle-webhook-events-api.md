...
3. Add a filter on **Client ID** matching the client ID of your OAuth Server-to-Server credential. Event filtering is based on `requestorclientid`—the same field described in [Field descriptions](#field-descriptions)—which identifies the client that submitted the job.
